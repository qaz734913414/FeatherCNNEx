...
void conv1x1s1_neon(float *input, int inch, int h, int w, int inChannelSize, float *output, int outch, int outh, int outw, int outChannelSize, const float* kernel, const float* bias, unsigned num_threads)
{
    ...
    for (int pp=0; pp<nn_outch; pp++)
    {
        ...
        float *out2 = output + (p+2)*outChannelSize;
        float *out3 = output + (p+3)*outChannelSize;
        float *out4 = output + (p+4)*outChannelSize;
        float *out5 = output + (p+5)*outChannelSize;
        float *out6 = output + (p+6)*outChannelSize;
        float *out7 = output + (p+7)*outChannelSize;
        ...
        fill(out2, outChannelSize, bias2);
        fill(out3, outChannelSize, bias3);
        fill(out4, outChannelSize, bias4);
        fill(out5, outChannelSize, bias5);
        fill(out6, outChannelSize, bias6);
        fill(out7, outChannelSize, bias7);
        ...
        for (; q+7<inch; q+=8)
        {
            ...
            const float* img1 = input + (q+1)*inChannelSize;
            const float* img2 = input + (q+2)*inChannelSize;
            const float* img3 = input + (q+3)*inChannelSize;
            const float* img4 = input + (q+4)*inChannelSize;
            const float* img5 = input + (q+5)*inChannelSize;
            const float* img6 = input + (q+6)*inChannelSize;
            const float* img7 = input + (q+7)*inChannelSize;
    ...
    for (int pp=0; pp<nn_outch; pp++)
    {
        ...
        for (; q<inch; q++)
        {
            ...
            const float* img0 = input + (q+0)*inChannelSize;
    ...
    for (int pp=0; pp<nn_outch; pp++)
    {
        ...
        float * out0 = output + (p+0)*outChannelSize;
        float * out1 = output + (p+1)*outChannelSize;
        float * out2 = output + (p+2)*outChannelSize;
        float * out3 = output + (p+3)*outChannelSize;
    ...
    {
        float * out = output + (p+0)*outChannelSize;
